...
const parseTransactionDate = (rawDateStr: string | null): Date | null => {
  ...
  // 3. Manual parsing for DD/MM/YYYY, MM/DD/YYYY, YYYY/MM/DD with slash/dash/dots
  ...
  // 4. Try month name mapping as final fallback (e.g. "29 Jun 2026" or "June 29, 2026")
  ...
  for (let i = 0; i < months.length; i++) {
    if (strLower.includes(months[i])) {
      ...
      if (digits && digits.length >= 2) {
        const year = parseInt(digits.find(d => d.length === 4) || "2026", 10);
        const day = parseInt(digits.find(d => d.length <= 2) || "1", 10);
...
export default function TrainerDashboard() {
  ...
  const [activeView, setActiveView] = useState<"virtual" | "west" | "analytics" | "status">("virtual");
  ...
  const getProcessedProductData = (): ProductTypeCount[] => {
    ...
    return productData.map(p => {
      // If transactions is undefined or we're in 'all' time (default), we use pre-aggregated values
      if (!p.transactions || p.transactions.length === 0 || analyticsDateRange === "all") {
        return p;
      }

      // Otherwise, let's filter transactions by date
      const filteredTx = p.transactions.filter(tx => {
        const d = parseTransactionDate(tx.date);
        if (!d) return false; // skip row with missing/invalid date if filtering is active
        return isDateInPeriod(d, analyticsDateRange, customStartDate, customEndDate);
      });
  ...
  const getRegionVsTrainingModeData = () => {
    // If date range is 'all', let's use the default westData.rows (the regional performance dataset), which has canonical numbers.
    // If a date range is active, we dynamically aggregate it from the transactional data!
    if (analyticsDateRange === "all" || !productData || productData.length === 0) {
      const data = {
    ...
    // Dynamic aggregation from filtered transactional data
    const data = {
    ...
    productData.forEach(p => {
      ...
      p.transactions.forEach(tx => {
        const d = parseTransactionDate(tx.date);
        if (!d || !isDateInPeriod(d, analyticsDateRange, customStartDate, customEndDate)) return;
