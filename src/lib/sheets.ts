...
export async function fetchWestZonePerformanceData(
  ...
  try {
    ...
    const isRawTransactions = header.includes("activity created on") || header.includes("createdby");
    ...
    if (isRawTransactions) {
      ...
      const modeIdx = header.findIndex(h => h === "training mode" || h.includes("mode"));
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (lowerMode.includes("physical")) {
          agg.physicalVisit++;
        } else if (lowerMode.includes("virtual")) {
          agg.virtual++;
        } else if (lowerMode.includes("out of station") || lowerMode.includes("oos")) {
          agg.outOfStation++;
        } else {
          agg.virtual++;
        }
      ...
      aggMap.forEach((agg) => {
        ...
        const productivityDay = (agg.physicalVisit / 3) + (agg.virtual / 5) + agg.outOfStation;
...
export async function fetchProductTypeData(
  ...
  try {
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      const dateVal = colDate !== -1 && row.length > colDate ? (row[colDate] || "").trim() : null;
